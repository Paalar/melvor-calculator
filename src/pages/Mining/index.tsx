...
import { OreNames, xpPerOre, respawnPerOre, pickaxeTimes, unlockables } from "./data";

const potionSpeeds = [1, 1.1, 1.2, 1.4, 1.8];

const Mining: FC = () => {
    const { calculatorState: { itemMastery, masteryPool } } = useContext(CalculatorContext);
    ...
    useEffect(() => {
        setXps(calculateXPperSecond(ore, 3 * timeReduction, Number(itemMastery[0]), potionLevel, Number(masteryPool)));
    ...
    return (
        <>
            ...
            <Calculator xps={xps} maxMastery={1089} items={[{ spa: 3 * timeReduction, name: ore }]} unlockables={unlockables} />
        </>
    );
}

const calculateXPperSecond = (oreType: OreNames, miningTime: number, oreMasteryLevel: number, potion: number, masteryLevel: number) => {
    ...
    const percentOfTimeActuallyMining = (oreHealth * potion) / ((respawnTime * mastery25) + oreHealth * potion);
